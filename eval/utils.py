...
from transformers import BitsAndBytesConfig, StoppingCriteria
...
def load_hf_lm_and_tokenizer(
    ...
    if gptq_model:
        ...
        if torch.cuda.is_available() and not load_in_4bit and not load_in_8bit:
            model = model.cuda()
        if convert_to_half:
            model = model.half()
    ...
    if peft_name_or_path is not None:
        print(f'Loading PEFT {peft_name_or_path}')
    ...
    for n, p in model.named_parameters():
        print(f'{n}: {p.dtype}, {p.size()}, {p.requires_grad}')
